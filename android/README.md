...
WakeMove 是一个 Android 10（API 29）及以上可用的本地闹钟原型。闹钟响起后，需要完成深蹲、开合跳、双手举起或随机中文语音口令才能正常停止；没有普通“关闭闹钟”按钮。每个闹钟可以独立选择 4 段内置原创舒缓铃声、3 种震动节奏和 3 档震动力度，并可在保存前试听和试震。中文语音口令由 APK 内置的 Vosk 离线模型识别，不依赖手机的系统语音服务，也不会在使用时下载模型。
...
当前发布 APK 是通用包，包含 ARM 32/64 位和 x86/x86_64 原生库，可直接分发
给不同 Android 设备。正式上架应用商店时建议改用 AAB，由商店按设备下发 ABI，
避免每位用户下载全部原生库。
